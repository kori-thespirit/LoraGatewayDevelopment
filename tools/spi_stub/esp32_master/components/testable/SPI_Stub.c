...
/* Private variables ---------------------------------------------------------*/
typedef struct SPI_Stub_Context_t {
  SPI_config cfg;
...
static esp_err_t Check_Pointer(SPI_Stub_handle_t *ctx) {
  if (ctx == NULL || *ctx == NULL) {
    return ESP_ERR_INVALID_ARG;
  }
  return ESP_OK;
}
static void cs_high(SPI_Stub_handle_t *ctx) {

  // ESP_LOGI(tag, "CS HIGH");
  gpio_set_level(((*ctx)->cfg.CS), 1);
}
static void cs_low(SPI_Stub_handle_t *ctx) {

  // ESP_LOGI(tag, "CS LOW");
  gpio_set_level(((*ctx)->cfg.CS), 0);
}
static esp_err_t SPI_init_bus(SPI_Stub_handle_t *ctx, uint16_t max_transfer);
static esp_err_t SPI_add_device(SPI_Stub_handle_t *ctx, SPI_config *cfg);
...
esp_err_t SPI_Stub_Init(SPI_Stub_handle_t *ctx, SPI_config *cfg) {
  ...
  if (*ctx != NULL) {
    ESP_LOGE(tag, "Already SPI_Stub Init\n");
    return ESP_ERR_INVALID_ARG;
  }
  *ctx = malloc(sizeof(SPI_Stub_Context_t));
  if (*ctx == NULL) {
    ESP_LOGE(tag, "khong the cap phat bo nho\n");
    return ESP_ERR_NO_MEM;
  }
  // memset((*ctx), 0, sizeof(SPI_Stub_Context_t));
  ...
  return ESP_OK;
clean:
  ESP_LOGE(tag, "Failed to Init device: %s", esp_err_to_name(err));
...
esp_err_t SPI_Stub_Set_Up(SPI_Stub_handle_t *ctx) {
  esp_err_t err;
  SPI_Stub_Register reg;
  memset(&reg, 0, sizeof(SPI_Stub_Register));
  reg.addr_reg = 0x37;
  reg.hummidity = 0x30;
  reg.temperature = 0x40;
  printfnumbinary((uint8_t *)&reg, sizeof(reg));
  err = Send_SPI_Stub_Register(ctx, (uint8_t *)&reg, sizeof(reg));
  return err;
}
esp_err_t Send_SPI_Stub_Register(SPI_Stub_handle_t *ctx, uint8_t *data,
  ...
  err = Check_Pointer(ctx);
  if (err != ESP_OK)
    ESP_LOGE(tag, "Error SPI_Stub Handle NULL\n");
  return err;
