...
// ============================================================================
// ĐỊA CHỈ VP CHO BÀN PHÍM CHỮ VÀ MẬT KHẨU
// ============================================================================
#define VP_KEYBOARD_TEXT 0x3000
#define VP_DISPLAY_TEXT 0x3300
#define VP_KEYBOARD_PASSWORD 0x4000
#define VP_DISPLAY_PASSWORD 0x3400
...
static const int maxTextLength = 50; // giới hạn ký tự nhập
// ============================================================================
// CẤU TRÚC DỮ LIỆU
// ============================================================================
...
// ============================================================================
// GỬI CHUỖI TEXT LÊN DWIN
// ============================================================================
void sendTextToDWIN(uint16_t address, const std::string &text)
{
    ...
    ESP_LOGI(TAG, "[TEXT] Gui len DWIN VP 0x%04X: %s", address, text.c_str());
}
// ============================================================================
// CÁC HÀM GỬI DỮ LIỆU
// ============================================================================
...
void sendGD20Temperature(float temperature)
{
    uint16_t tempValue = (uint16_t)(temperature * 100);
    setVPWord(ADDRESS_GD20_TEMP, tempValue);
    ESP_LOGI(TAG, "Da gui nhiet do GD20: %.1f°C (gia tri: %d)", temperature, tempValue);
}

void sendGD20Amp(float temperature)
{
    uint16_t tempValue = (uint16_t)(temperature * 100);
    setVPWord(ADDRESS_GD20_AMP, tempValue);
    ESP_LOGI(TAG, "Da gui nhiet do GD20: %.1f°C (gia tri: %d)", temperature, tempValue);
}

void sendGD20Volt(float temperature)
{
    uint16_t tempValue = (uint16_t)(temperature * 100);
    setVPWord(ADDRESS_GD20_VOLT, tempValue);
    ESP_LOGI(TAG, "Da gui nhiet do GD20: %.1f°C (gia tri: %d)", temperature, tempValue);
}

void sendGD20Frequency(float temperature)
{
    uint16_t tempValue = (uint16_t)(temperature * 100);
    setVPWord(ADDRESS_GD20_FREQ, tempValue);
    ESP_LOGI(TAG, "Da gui nhiet do GD20: %.1f°C (gia tri: %d)", temperature, tempValue);
}
...
void updateTemperature()
{
    if (sensorData.isSystemRunning)
    {
        ...
        sendGD20Temperature(sensorData.temperature);
        sendGD20Amp(sensorData.temperature);
        sendGD20Volt(sensorData.temperature);
        sendGD20Frequency(sensorData.temperature);
    }
}
            ...
            if (data[i] == 0x5A && data[i + 1] == 0xA5)
            {
                ...
                if (cmd == 0x83)
                {
                    ...
                    if (dataLen >= 1)
                    {
                        ...
                        // ============================================================
                        // XỬ LÝ BÀN PHÍM CHỮ (VP 0x3000)
                        // ============================================================
                        else if (address == VP_KEYBOARD_TEXT)
                        {
                            ESP_LOGI(TAG, "─────────────────────────────────────────────");
                            ESP_LOGI(TAG, "[RAW] KeyCode: 0x%02X", lastByte);
                            ...
                            if (lastByte == 0x00F2)
                            {
                                ...
                                {
                                    pendingText.pop_back();
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[XOA CHU] Backspace - Xoa 1 ky tu");
                                }
                            }
                            // Clear
                            else if (lastByte == 0x00F5)
                            {
                                pendingText.clear();
                                sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                ESP_LOGI(TAG, "[XOA CHU] Clear - Xoa toan bo");
                            }
                            // Enter / Confirm
                            else if (lastByte == 0x00F1)
                            {
                                ESP_LOGI(TAG, "[CONFIRM CHU] Chuoi xac nhan: %s", pendingText.c_str());
                                // Có thể thêm hành động lưu text vào đây
                            }
                            // Cancel
                            else if (lastByte == 0x00F0)
                            {
                                pendingText.clear();
                                sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                ESP_LOGI(TAG, "[CANCEL CHU] Huy bo nhap lieu");
                            }
                            // Số 0-9
                            else if (lastByte >= 0x30 && lastByte <= 0x39)
                            {
                                char so = '0' + (lastByte - 0x30);
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back(so);
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] So: %c", so);
                                }
                            }
                            // Chữ hoa A-Z
                            else if (lastByte >= 0x41 && lastByte <= 0x5A)
                            {
                                char ch = (char)lastByte;
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back(ch);
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] Chu hoa: %c", ch);
                                }
                            }
                            // Chữ thường a-z
                            else if (lastByte >= 0x61 && lastByte <= 0x7A)
                            {
                                char ch = (char)lastByte;
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back(ch);
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] Chu thuong: %c", ch);
                                }
                            }
                            // Ký tự đặc biệt (mở rộng)
                            // Xử lý ký tự đặc biệt (dựa trên bảng mã ASCII)
                            else if (lastByte == 0x20)
                            { // SPACE
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back(' ');
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] Space");
                                }
                            }
                            else if (lastByte == 0x21)
                            { // !
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back('!');
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] !");
                                }
                            }
                            else if (lastByte == 0x22)
                            { // "
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back('"');
                            ...
                            else if (lastByte == 0x5F)
                            { // _
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back('_');
                                    sendTextToDWIN(VP_DISPLAY_TEXT, pendingText);
                                    ESP_LOGI(TAG, "[NHAP CHU] _");
                                }
                            }
                            else if (lastByte == 0x60)
                            { // `
                                if ((int)pendingText.size() < maxTextLength)
                                {
                                    pendingText.push_back('`');
                            ...
                            {
                                // Có thể thêm các ký tự đặc biệt nếu cần
                                ESP_LOGI(TAG, "[UNKNOWN] Key code khong xu ly: 0x%02X", lastByte);
                            }
                            ESP_LOGI(TAG, "─────────────────────────────────────────────");
                        }

                        // ============================================================
                        // XỬ LÝ BÀN PHÍM MẬT KHẨU (VP 0x4000)
                        // ============================================================
                        else if (address == VP_KEYBOARD_PASSWORD)
                        {
                            ESP_LOGI(TAG, "─────────────────────────────────────────────");
                            ESP_LOGI(TAG, "[RAW PASSWORD] KeyCode: 0x%02X", lastByte);
                            ...
                            if (lastByte == 0x00F2)
                            {
                                ...
                                {
                                    pendingPassword.pop_back();
                                    sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                    ESP_LOGI(TAG, "[XOA PASSWORD] Backspace - Xoa 1 ky tu");
                                }
                            }
                            // Clear
                            else if (lastByte == 0x00F5)
                            {
                                ...
                                sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                ESP_LOGI(TAG, "[XOA PASSWORD] Clear - Xoa toan bo");
                            }
                            // Enter / Confirm
                            else if (lastByte == 0x00F1)
                            {
                                ESP_LOGI(TAG, "[CONFIRM PASSWORD] Chuoi xac nhan: %s", pendingPassword.c_str());
                            ...
                            else if (lastByte == 0x00F0)
                            {
                                pendingPassword.clear();
                                sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                ESP_LOGI(TAG, "[CANCEL PASSWORD] Huy bo nhap lieu");
                            }
                            // Số 0-9
                            else if (lastByte >= 0x30 && lastByte <= 0x39)
                            {
                                char so = '0' + (lastByte - 0x30);
                                if ((int)pendingPassword.size() < maxTextLength)
                                {
                                    pendingPassword.push_back(so);
                                    sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                    ESP_LOGI(TAG, "[NHAP PASSWORD] So: %c", so);
                                }
                            }
                            // Chữ hoa A-Z
                            else if (lastByte >= 0x41 && lastByte <= 0x5A)
                            {
                                ...
                                if ((int)pendingPassword.size() < maxTextLength)
                                {
                                    pendingPassword.push_back(ch);
                                    sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                    ESP_LOGI(TAG, "[NHAP PASSWORD] Chu hoa: %c", ch);
                                }
                            }
                            // Chữ thường a-z
                            else if (lastByte >= 0x61 && lastByte <= 0x7A)
                            {
                                ...
                                if ((int)pendingPassword.size() < maxTextLength)
                                {
                                    pendingPassword.push_back(ch);
                                    sendTextToDWIN(VP_DISPLAY_PASSWORD, pendingPassword);
                                    ESP_LOGI(TAG, "[NHAP PASSWORD] Chu thuong: %c", ch);
                                }
                            }
                            // Xử lý ký tự đặc biệt (dựa trên bảng mã ASCII)
                            else if (lastByte == 0x20)
                            ...
                            {
                                ESP_LOGI(TAG, "[UNKNOWN PASSWORD] Key code: 0x%02X", lastByte);
                            }
                        }
                        ESP_LOGI(TAG, "======================================================================");
                    }
...
#pragma endregion HMIEvent

#pragma region FREERTOS Task
// ============================================================================
// FREE RTOS TASKS
// ============================================================================
...
#pragma endregion FREERTOS Task
// ============================================================================
// KHỞI TẠO DỮ LIỆU
// ============================================================================
